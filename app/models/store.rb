class Store < ActiveRecord::Base
	...
	def self.to_csv
	 CSV.generate do |csv|
	 	# csv << column_names
	 	csv << ["id", "name", "address", "phone", "email", "url", "description", "categories", "custom_field_1", "custom_field_2", "custom_field_3", "image_url", "custom_marker_url", "lat", "long", "created_at", "updated_at"]
	  	...
	  	all.each do |store|
	 		...
	 		csv << [store.id,store.name, store.address, store.phone, store.email, store.url, store.description, store.categories, store.custom_field_1, store.custom_field_2, store.custom_field_3, store.image_url, store.custom_marker_url, store.lat, store.long, store.created_at, store.updated_at]
	 	end
	 end
	end
end
